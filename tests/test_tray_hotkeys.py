...
def test_tray_answer_hotkey_records_background_failure(monkeypatch, tmp_path):
    calls = []
    callbacks = {}
    paths = runtime_paths(tmp_path)
    ...
    assert paths.status_file.read_text(encoding="utf-8") == "answer failed: backend unavailable"
    assert calls == ["listener.stop"]
